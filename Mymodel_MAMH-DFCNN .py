...
class SELayer(nn.Layer):
    ...
    def forward(self, x):
        ...
        return out
class FAE(nn.Layer):
    def __init__(self, batch_size, Def, stride1, stride2, kernel_size1, kernel_size2, aver_pool, input_channels,
                 output_dim):
        ...
        self.k1 = paddle.ones(shape=[input_channels * Def])
        self.k1.stop_gradient = True
        self.k2 = paddle.ones(shape=[input_channels * Def])
        self.k2.stop_gradient = True
        ...
        if aver_pool:
            self.decoder = nn.Sequential(
                nn.Conv2DTranspose(in_channels=1, out_channels=input_channels, kernel_size=kernel_size1, stride=stride1,
                                   padding=0),
                nn.GELU(),
                nn.Conv2DTranspose(in_channels=input_channels, out_channels=input_channels, kernel_size=kernel_size2,
                                   stride=stride2, padding=0),
                nn.GELU()
            )
        else:
            self.decoder = nn.Sequential(
